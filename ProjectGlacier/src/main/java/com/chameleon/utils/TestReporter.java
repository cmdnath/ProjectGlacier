...
public class TestReporter {
    ...
    public static void setAssertFailed(Boolean value) {
        assertFailed.set(value);
    }
        ...
        if (getPrintFullClassPath()) {
            ...
            for (StackTraceElement element : elements) {
                ...
                if (x == 0 || x == 1 || x == 2) {
                    x++;
                    continue;
                } else if (!filename.matches("sun.reflect|com.chameleon.utils.TestReporter|com.chameleon.utils.TestReporter|com.chameleon.utils.PageLoaded|java.lang.reflect|"
                        + "java.lang.Thread|com.sun.proxy|org.testng.internal|java.util.concurrent.ThreadPoolExecutor|com.chameleon.utils.debugging")) {
    ...
    public synchronized static void assertEquals(Object value1, Object value2, String description) {
        try {
            ...
            logFailure("Assert Equals - " + value1.toString() + " = " + value2.toString() + " - " + description);
            if (getPrintToConsole()) {
                System.out.println(getTimestamp() + "Assert Equals - " + trimHtml(description) + " - " + value1.toString() + " = " + value2.toString());
            }
            Assert.fail(description);
        }
        Reporter.log(getTimestamp() + " <font size = 2 color=\"green\"><b><u>Assert Equals - " + description + " - " + value1 + " = " + value2 + "</font></u></b><br />");
        if (getPrintToConsole()) {
            System.out.println(getTimestamp() + "Assert Equals - " + trimHtml(description) + " - " + value1 + " = " + value2);
        }
    }
    ...
    public synchronized static void assertNotEquals(Object value1, Object value2, String description) {
        try {
            ...
            logFailure("Assert Not Equals - " + value1 + " != " + value2 + " - " + description);
            if (getPrintToConsole()) {
                System.out.println(getTimestamp() + "Assert Not Equals - " + value1 + " != " + value2 + " - " + trimHtml(description));
            }
            Assert.fail(description);
        }
        Reporter.log(getTimestamp() + " <font size = 2 color=\"green\"><b><u>Assert Not Equals - " + value1 + " != " + value2 + " - " + description + "</font></u></b><br />");
        if (getPrintToConsole()) {
            System.out.println(getTimestamp() + "Assert Not Equals - " + value1 + " != " + value2 + " - " + trimHtml(description));
        }
    }
    ...
    public synchronized static boolean softAssertEquals(Object value1, Object value2, String description) {
        ...
        try {
            ...
            Reporter.log(getTimestamp() + " <font size = 2 color=\"green\"><b><u>Assert Equals - " + description
                    + ": [ " + value1 + " ] = [ " + value2 + " ]</font></u></b><br />");
            if (getPrintToConsole()) {
                System.out.println(getTimestamp() + "Assert Equals - " + trimHtml(description));
            }
        } catch (AssertionError failure) {
            Reporter.log(getTimestamp() + "<font size = 2 color=\"red\"><b><u>Assert Equals - " + description + ": [ " + value1 + " ] = [ " + value2 + " ]</b></u></font><br />");
            if (getPrintToConsole()) {
                System.out.println(getTimestamp() + "Assert Equals - " + trimHtml(description));
            }
            assertFailed.set(true);
    ...
    public synchronized static boolean assertAll() {
    ...
    public synchronized static void logScreenshot(String fileName) {
        logScreenshot(DriverManager.getDriver(), fileName);
    }

    public synchronized static void logScreenshot(By by, String fileName) {
        ...
        logScreenshot(by, destFile, slash);

    }

    public synchronized static void logScreenshot(Element element, String fileName) {
        String slash = Constants.DIR_SEPARATOR;
        ...
        String destFile = destDir + slash + dateFormat.format(new Date()) + ".png";
        logScreenshot(element, destFile, slash);
    }
    ...
    public synchronized static void logScreenshot(WebDriver driver, String fileLocation, String slash) {
        ...
        TestReporter.logTrace("File location on local: " + fileLocation);
        ...
        if (isNotEmpty(jenkinsPath)) {
            ...
            String webFileLocation = fileLocation.replace(jenkinsWorkspace, jenkinsPath + "ws/");

            TestReporter.logTrace("Web File Location : " + webFileLocation);
            Reporter.log("<a  target='_blank' href='" + webFileLocation + "'><img src='" + webFileLocation + "' height='200' width='300'/></a></br></br>");
        } else {
            TestReporter.logInfo("File Location : " + fileLocation);
            Reporter.log("<a  target='_blank' href='" + fileLocation + "'> <img src='file:///" + fileLocation + "' height='200' width='300'/> </a> </br></br>");
        }
    }

    public synchronized static void logScreenshot(Element element, String fileLocation, String slash) {
        try {
            FileUtils.copyFile(element.getScreenshotAs(OutputType.FILE), new File(fileLocation));
        } catch (IOException e) {
            TestReporter.log("Screenshot was not captured - IOException: " + e.getMessage());
        } catch (TestNGException te) {
            TestReporter.log("Screenshot was not captured - TestNGException: " + te.getMessage());
        ...
        if (isNotEmpty(jenkinsPath)) {
            ...
            TestReporter.logTrace("Web File Location : " + webFileLocation);
            Reporter.log("<a  target='_blank' href='" + webFileLocation + "'><img src='" + webFileLocation + "' height='" + height + "' width='" + width + "'/></a></br>");
        } else {
            TestReporter.logInfo("File Location : " + fileLocation);
            Reporter.log("<a  target='_blank' href='" + fileLocation + "'> <img src='file:///" + fileLocation + "' height='" + height + "' width='" + width + "'/></a></br>");
        }
    }

    public synchronized static void logScreenshot(By by, String fileLocation, String slash) {
        File file = new File("");

        try {
            file = DriverManager.getDriver().findElement(by).getScreenshotAs(OutputType.FILE);
            FileUtils.copyFile(file, new File(fileLocation));
        } catch (IOException e) {
            TestReporter.log("Screenshot was not captured - IOException: " + e.getMessage());
        } catch (TestNGException te) {
